...
while True:
    ...
    if pitft.Button1:
        print "BTN 1 KILL CHROMIUM"
        # subprocess.call("sudo killall chromium-browser")
        os.system("sudo killall chromium-browser")
    if pitft.Button2:
        print "BTN 2 LAUNCH CHROMIUM"
        # DISPLAY=:0 sudo python python_script.py
        chromium_cmd = 'DISPLAY=:0 sudo chromium-browser --disable-setuid-sandbox --kiosk --noerrdialogs --disable-session-crashed-bubble --disable-infobars --disable-web-security --user-data-dir localhost'
        os.system(chromium_cmd)
        # subprocess.call("sudo killall chromium-browser")
        # subprocess.call(["chromium-browser", " --disable-setuid-sandbox", "--kiosk", "--noerrdialogs", "--disable-session-crashed-bubble", "--disable-infobars", "--disable-web-security", "--user-data-dir", "localhost"])
    if pitft.Button3:
        print "BTN 3 REBOOT"
        # subprocess.call("sudo killall chromium-browser", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        os.system("sudo reboot")
    if pitft.Button4:
        Backlight(False)
        print "BTN 4 POWER OFF"
